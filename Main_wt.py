#!/usr/bin/env python3
from Model_wt import TGVModel_wt as TGVModel
...
for e in [1, 1, 3, 5, 10, 15, 15, 25, 25, 25, 25, 25, 25]:
    ...
    m.model.fit([word_e[:1530], tags_e[:1530]], desired[:1530], batch_size=batch_s, nb_epoch=e)
    epochs += e
    # testing data
    golden_values = [x * 2 - 1 for x in golden[1530:]]
    # use cross-validation later
    prediction = m.predict([word_e[1530:], tags_e[1530:]]) * 2 - 1
